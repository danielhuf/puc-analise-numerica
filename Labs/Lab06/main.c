...
void teste_adaptativo(double (*f)(double), double a, double b, double d) {
  for (int i = 1; i < 13; ++i) {
    double tol = pow(10, -i);
    double s = simpsonadaptativo(
        f, a, b, tol); // COMPLETAR: chamar simpson adaptativo em [a,b]
    printf("%e %.16g %e\n", tol, s, fabs(d - s));
  }
}

static double fd1(double x) { return cos(x) - 2 * sin(x); }
static double fd1l(double x) { return -sin(x) - 2 * cos(x); }
static double fd2(double x) { return exp(x); }
static double fd2l(double x) { return exp(x); }
static double fi1(double x) { return x / sqrt(x * x + 9); }
static double fi2(double x) { return x * x * sin(x); }
static double fi3(double x) { return (2 / sqrt(PI)) * exp(-(x * x)); }
static double fi4(double x) { return log(cos(x) + sin(x)); }
